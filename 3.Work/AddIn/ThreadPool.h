...
BOOL CMyThreadPool<T>::Create(INT nCount)
{
    ...
    __try
    {
        if(nCount <= 0)
        {
            TRACE(TEXT("CMyThreadPool::Create: 线程不能为0 或小于0"));
            bRet = FALSE;
        }
        ...
        {
            TRACE(TEXT("CMyThreadPool::Create: 创建信号量失败 : %s"), CMyThreadPool<T>::GetErrorMsg());
            bRet = FALSE;
        }
        ...
        {
            TRACE(TEXT("CMyThreadPool::Create: 创建线程数组失败!"));
            bRet = FALSE;
        }
        ...
        for(int i = 0; i < nCount; i++)
        {
            ...
            {
                TRACE(TEXT("CMyThreadPool::Create: 创建线程失败"));
                nFailCount++;
                if(nFailCount < 5)
                {
                    TRACE(TEXT("CMyThreadPool::Create: 创建线程失败,重试中 %d 次, 错误信息 : %s"), nFailCount, GetErrorMsg());
                    continue;
                }
                else
                {
                    ...
                    delete m_pThreadAry;
                    m_pThreadAry = NULL;
                    TRACE(TEXT("CMyThreadPool::Create: 创建线程失败，重试超次。"));
                    bRet = FALSE;
                }
            }
    ...
    __finally
    {
        if(!bRet)
        {
            m_strErrStr = TEXT("CMyThreadPool::Create: 线程池创建失败，请打开调试输出信息，进行查看");
        }
    }
...
DWORD AFX_CDECL CMyThreadPool<T>::ThreadFun(LPVOID lParam)
{
    ...
    if(lParam == NULL)
    {
        TRACE(TEXT("CMyThreadPool::ThreadFun: 线程中：This 指针 指向0地址"));
        return FALSE;
    }
    ...
    while(TRUE)
    {
        ...
        if(dwRet == WAIT_FAILED)
        {
            ...
            TRACE(TEXT("CMyThreadPool::ThreadFun: 创建线程失败，重试超次。"));
        ...
        {
            TRACE(TEXT("CMyThreadPool::ThreadFun: 线程退出"));
            AfxEndThread(0);
        }
    }//End while(true)
...
BOOL CMyThreadPool<T>::Destroy()
{
    ...
    {
        //TRACE(TEXT("CMyThreadPool::Destroy: 多次进入销毁线程池"));

        m_bIsFirstIntoDestroy = FALSE;

        if(m_pThreadAry == NULL)
        {
            TRACE(TEXT("CMyThreadPool::Destroy: 线程句柄数组为空, 不需要销毁线程池"));
            return FALSE;
        }
        ...
        {
            TRACE(TEXT("CMyThreadPool::Destroy: 等待所有线程退出，超时或错误 : %s"), GetErrorMsg());
            return FALSE;
        }
        else if(dwRet == WAIT_FAILED)
        {
            TRACE(TEXT("CMyThreadPool::Destroy: 等待所有线程退出，WAIT_FAILED : %s"), GetErrorMsg());
            return FALSE;
        }
...
//     }
    ...
    TRACE(TEXT("CMyThreadPool::Destroy: 销毁线程池成功"));
...
BOOL CMyThreadPool<T>::Handle(T* pCmd)
{
    if(pCmd == NULL)
    {
        TRACE(TEXT("CMyThreadPool::Handle: 传入指令为空"));
        return FALSE;
    }
...
BOOL CPoolManager<T>::Execute()
{
    ...
    __try
    {
        ...
        if(m_CmdList.IsEmpty())
        {
            TRACE(TEXT("CPoolManager::Manager：队列为空"));
            bRet = TRUE;
            __leave;
        }
